/*
This script is a modified version of dpryan79' extractSoftclipped.c
in SE-MEI [https://github.com/dpryan79/SE-MEI] at commit abb8a8a.

The modification includes:
1. output fasta instead of fastq
2. output the name of the query instead of the reference
3. output the reverse complementary sequence if the alignment is RC
...
void process_alignment(bam1_t *b, bam_hdr_t *hdr, FILE *of, int length) {
    ...
    char int2char[16] = {'N', 'A', 'C', 'N', 'G', 'N', 'N', 'N',
                         'T', 'N', 'N', 'N', 'N', 'N', 'N', 'N'};
    char int2char_comp[16] = {'N', 'T', 'G', 'N', 'C', 'N', 'N', 'N',
                         'A', 'N', 'N', 'N', 'N', 'N', 'N', 'N'};
    for(i=0; i<b->core.n_cigar; i++) {
        ...
        if(op != 4 || op_len < length) {
            ...
            fprintf(of, ">%s:%"PRId32"\n", /*hdr->target_name[b->core.tid]*/ bam_get_qname(b), pos);
            if(bam_is_rev(b)) {
              for(j=op_len-1; j >= 0; --j) {
                  fprintf(of, "%c", int2char_comp[bam_seqi(bam_get_seq(b), j+apos)]);
              }
            } else {
              for(j=0; j < op_len; j++) {
...
void usage(char *prog) {
    ...
    printf("\n\
This program has been modified by bowhan to: \n\
1. output fasta instead of fastq \n\
2. output the name of the query instead of the reference \n\
3. output the reverse complementary sequence if the alignment is RC. \n\
